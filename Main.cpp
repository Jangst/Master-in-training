...
void encryptStr() {
	...
	do {
		try {
			std::cout << " Input 1 to encrypt, 2 to decrypt, and 0 to end. " << std::endl;;
			std::cin >> inputCommand;
			if (inputCommand > 2 || inputCommand < 0) {		//exception verifying input
				throw 1;
			}
		}
		catch (...) {
		...
		if (inputCommand == 1) {
			...
			if (EN.getValidInput(inputPhrase) == true) {		//checking input is valid string
				...
				std::cout << " Input proper phrase " << std::endl;
			...
			if (inputCommand == 0) continue;
			std::cout << " Use proper input " << std::endl;
		}
		system("pause");
		system("CLS");				//reset screen for continuous use
	} while (inputCommand != 0);

	std::cout << " Have a nice day :D " << std::endl;	//ending on a happy note
}
